...
#include <app/clusters/operational-credentials/operational-credentials.h>
...
class ServerStorageDelegate : public PersistentStorageDelegate
{
    ...
    {
        ChipLogProgress(AppServer, "Stored value in server storage, writing over fabrics list attribute.");
...
CHIP_ERROR RestoreAllAdminPairingsFromKVS(AdminPairingTable & adminPairings, AdminId & nextAvailableId)
{
    ...
    ChipLogProgress(AppServer, "Restored all admin pairings from KVS, writing admins into fabrics list attribute.");
    writeAdminsIntoFabricsListAttribute();
